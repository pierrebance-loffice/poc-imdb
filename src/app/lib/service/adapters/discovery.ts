...
function toDiscovery(discovery: IApiDiscovery, genres: Record<number, string>): Discovery {
  const {
    id,
    title,
  ...
  return {
    ...
    overview,
    popularity,
    posterPath: discovery.poster_path ? toImages(discovery.poster_path) : undefined,
    video,
    voteAverage,
    voteCount,
