...
export default function MoviePeople({ movie }: { movie: Movie }) {
  const director = useMemo(() => movie.credits.crew.find(({ job }) => job === 'Director'), [movie])
  const actors = useMemo(
    () =>
      movie?.credits?.cast
        .sort((person1, person2) => (person1.popularity > person2.popularity ? -1 : 1))
        .filter((actor) => !!actor.character),
    [movie],
  )

  const persons = useMemo(() => sift([...(director ? [director] : []), ...actors]), [director, actors])

  const total = useMemo(() => Math.ceil(persons.length / creditsPagination), [persons])
  ...
  return (
    <>
      ...
      <div className="flex items-center gap-4 self-center">
        {persons.slice((page - 1) * creditsPagination, page * creditsPagination).map((person, index) => (
          <Link
          ...
          >
            {!!person?.profilePath && (
              <Image src={person?.profilePath?.small} alt="photo-mini" className="rounded" height={200} width={134} />
            )}
            ...
            <div className="absolute bottom-0 left-0 right-0 top-0">
              <div className="absolute bottom-0 left-0 right-0 top-0 text-ellipsis p-2 text-center text-lg font-bold text-white ">
                <div className="absolute bottom-0 left-0 right-0 top-0 p-2 text-center opacity-0 hover:opacity-100">
                  ...
                  {!!director && director.id === person.id && !!person.job && (
                    <div
                      data-testid="person-job"
                      className="absolute bottom-0 left-0 right-0 p-2 text-center text-xl font-bold"
                    >
                      <Genre name="Director" className="inline text-ellipsis p-2 text-xs font-bold" />
                    </div>
                  )}
                </div>
              </div>
