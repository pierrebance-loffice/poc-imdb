...
export default function PersonModal({ person, movie }: { person: Person; movie?: Movie }) {
  ...
  return (
    <Modal>
      <PersonCard person={person} compact={true} personRole={person ? `${movie?.title}: ${role}` : role} />
      <span className="grow" />
      <Link href={`/people/${person.id}`} className="self-center text-blue-500 hover:text-blue-800">
